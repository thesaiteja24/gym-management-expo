...
export default function HabitCreatorScreen() {
	...
	const [trackingType, setTrackingType] = useState<'streak' | 'quantity'>(habitToEdit?.trackingType || 'streak')
	const [source, setSource] = useState<'manual' | 'internal'>(habitToEdit?.source || 'internal')
	const [internalMetricId, setInternalMetricId] = useState<string | null>(habitToEdit?.internalMetricId || null)
