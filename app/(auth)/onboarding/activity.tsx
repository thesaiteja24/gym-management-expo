			...
			<ScrollView contentContainerStyle={{ padding: 24, paddingBottom: 100 }}>
				...
				<View className="flex gap-4">
					{levels.map(lvl => (
						...
						>
							<Text
								className={`mt-1 px-1 text-sm ${activityLevel === lvl.value ? 'text-blue-800 dark:text-blue-200' : 'text-neutral-500 dark:text-neutral-400'}`}
							>
								{lvl.desc}
							</Text>
						</SelectableCard>
			...
			<View className="absolute bottom-0 left-0 right-0 flex items-center justify-center border-t border-neutral-100 bg-white p-6 dark:border-neutral-900 dark:bg-black">
				<View className="flex flex-row items-center justify-between gap-4 px-8">
					...
					<Button
						...
						onPress={handleNext}
						fullWidth
						rightIcon={<MaterialCommunityIcons name="magic-staff" size={24} color={colors.icon} />}
					/>
				</View>
			</View>
		</SafeAreaView>
