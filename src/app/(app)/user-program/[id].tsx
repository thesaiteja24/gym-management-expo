...
export default function UserProgramDashboard() {
  ...
  const activeWeekIndex = requestedWeekIndex ?? userProgram?.progress.currentWeek ?? 0
  const isWeekSwitchLoading = Boolean(requestedWeekIndex !== null && isFetching)
  const progressPercent = userProgram
    ? Math.round(
  ...
  const renderHeaderRight = () => (
    <Button
      ...
      leftIcon={<Ionicons name="settings-outline" size={24} color="#6366f1" />}
      className="p-0"
    />
  )
  ...
  const renderFooter = () => {
    ...
    return (
      <View className="absolute bottom-0 left-0 right-0 flex items-center bg-transparent p-4">
        <Button
          title={
            isRestDay ? 'View Rest Day' : hasWorkout ? "Start Today's Workout" : 'No Workout Today'
          }
          variant="primary"
    ...
    >
      ...
      <View className="mb-6">
        ...
        <View className="mt-2 flex-row items-center gap-2">
          ...
          <Text className="text-neutral-500">
            Week {(userProgram?.progress.currentWeek ?? 0) + 1} • Day{' '}
            {(userProgram?.progress.currentDay ?? 0) + 1}
          </Text>
        </View>
      </View>
      ...
      <View className="mb-4">
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} className="flex-row gap-2">
          {Array.from({ length: userProgram?.durationWeeks || 0 }).map((_, i) => (
            <TouchableOpacity
              key={i}
              onPress={() => {
