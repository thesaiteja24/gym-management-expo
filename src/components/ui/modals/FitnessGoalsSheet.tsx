...
export const FitnessGoalsSheet = forwardRef<BottomSheetModal, Props>(
  ({ persistOnNavigation = false }, ref) => {
    ...
    const [targetType, setTargetType] = useState<TargetType>(
      (fitnessProfile?.targetType as TargetType) || 'weight',
    )
    ...
    const computedTargets = useMemo(() => {
      if (!currentWeight || !height || !gender || !user?.dateOfBirth || !goalType || !finalRate)
        return null
        ...
        >
          ...
          {mode === 'dateDriven' && (
            <View className="mb-6">
              ...
              {calculatedRate && (
                <Text className="mt-3 text-sm text-neutral-500 dark:text-neutral-400">
                  Required weekly change: {calculatedRate}{' '}
                  {targetType === 'weight' ? weightUnit : '%'}
                </Text>
              )}
            </View>
          )}
