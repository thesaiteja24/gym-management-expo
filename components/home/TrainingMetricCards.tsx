	...
	return (
		...
		>
			<View className="flex-row items-center justify-between">
				<Text className="text-base font-medium text-neutral-600 dark:text-neutral-400">{title}</Text>
				<MaterialCommunityIcons name={icon} size={20} color={iconColor} />
			</View>

			<View>
				<Text className="text-2xl font-bold text-black dark:text-white">{value}</Text>
				{subtitle && <Text className="text-xs text-neutral-500 dark:text-neutral-500">{subtitle}</Text>}
			</View>
		</Pressable>
	)
}
...
export function WeeklyRepsCard({ reps, lastWeekReps, width }: { reps: number; lastWeekReps: number; width: number }) {
