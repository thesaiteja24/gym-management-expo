 ...
 */
export function enqueueTemplateCreate(
  payload: TemplatePayload,
  userId: string,
): TemplateMutation {
  const queue = getTemplateQueue();
  const existing = queue.find(
    (m) => m.clientId === payload.clientId && m.type === "CREATE",
  );
 ...
 */
export function enqueueTemplateUpdate(
  payload: TemplatePayload,
  userId: string,
): TemplateMutation {
  const queue = getTemplateQueue();
  ...
  const existingUpdateIdx = queue.findIndex(
    (m) => m.clientId === payload.clientId && m.type === "UPDATE",
  );
 ...
 */
export function enqueueTemplateDelete(
  clientId: string,
  dbId: string | null,
  userId: string,
  ...
  const existingCreateIdx = queue.findIndex(
    (m) => m.clientId === clientId && m.type === "CREATE",
  );
  ...
  const filteredQueue = queue.filter(
    (m) => !(m.clientId === clientId && m.type === "UPDATE"),
  );

  // Check if DELETE already exists
  const existingDelete = filteredQueue.find(
...
export function incrementTemplateRetry(queueId: string): void {
  const queue = getTemplateQueue().map((m) =>
    m.queueId === queueId ? { ...m, retryCount: m.retryCount + 1 } : m,
  );
  saveTemplateQueue(queue);
}
...
export function getTemplateQueueCounts(userId: string): {
  ...
  const failed = getTemplateFailedQueue().filter(
    (m) => m.userId === userId,
  ).length;
